...
const ServiceCategoryPanel = ({
  ...
  // State to handle rotation of the SVG icon
  const [isRotated, setIsRotated] = useState(false);

  // Function to handle the toggle of the panel and SVG rotation
  const handleToggle = () => {
    setIsRotated(!isRotated); // Toggle the rotation
    panelRef.current.toggle(); // Toggle the panel visibility
  };
